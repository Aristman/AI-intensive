...
🔥 День 26. Secure Agent

Добавьте авторизацию и ограничение доступа к вашему агенту. Используйте токены, роли, лимиты.

Результат: Агент с безопасным интерфейсом
Формат: Видео + Код

Вот пример промта, который можно использовать для доработки кодовой базы (ориентирован на LLM-ассистента, чтобы он аккуратно внедрил функционал авторизации, ролей и лимитов в интерфейс и реализации агентов):

---

**Промт для доработки кодовой базы**

Доработай проект следующим образом:

1. **Интерфейс `IAgent`:**

 * Добавь методы и/или свойства для работы с авторизацией:

  * `authenticate(token: str) -> bool` — проверка валидности токена.
  * `role: str` — роль текущего пользователя/подключения.
  * `limits: dict` — ограничения (например, число запросов, объем данных).

2. **Реализации агентов:**

 * Обнови все классы, которые реализуют `IAgent`, чтобы:

  * Поддерживалась аутентификация через токен.
  * Назначались роли (например, `"admin"`, `"user"`, `"guest"`) с разными правами доступа.
  * Проверялись лимиты (например, ограничение количества обращений в единицу времени).

3. **Подключение агентов:**

 * При установке соединения с агентом необходимо:

  * Выполнить авторизацию (токен обязателен).
  * Проверить роль и разрешить/запретить доступ к определённым функциям.
  * Учитывать лимиты — если превышены, агент должен возвращать ошибку/отказ в обслуживании.

4. **Общие требования:**

 * Сделай реализацию расширяемой, чтобы можно было легко добавлять новые роли и политики лимитов.
 * Обеспечь обратную совместимость: старый код должен работать, если явно не используется новая логика авторизации.
 * Добавь тесты для проверки аутентификации, ролей и лимитов.

---

🔥 День 28. Агент для Legacy-кода

Сделайте агента, который анализирует старый проект и предлагает, что улучшить: убрать дубли, повысить читаемость, найти устаревшие зависимости.

Результат: Агент, умеющий ревьюить код
Формат: Видео + Код


🔥 День 27. Personalization
...
---
Создай профиль пользователя приложения.
В профиль включи
Имя
Роль
Список из предпочтений (название и описание)
Список из исключений (название и описание)

На главном экране выдели имя пользователя
При клике по имени пользователя переходи на экран профиля
Создай экран профиля. В полях со списками сделай кнопки Добавить запись (открывается модальное окно для добавления записи) и напротив каждой существующей записи Удалить запись, есть возможность редактировать запись.
Вынеси из интерфейса @agent_interface.dart данные о пользователе, добавь в этот интерфейс поле Пользователь с профилем пользователя. Используй данные из профиля пользователя в реализациях интерфеса (где требуется)

@create-roadmap.md @readme-roadmap.md @tests.md 
---

**Промпт для разработчика ИИ Агента**

Требуется доработать приложение, добавив функционал профиля пользователя.

### Задачи:

1. **Создать профиль пользователя приложения**, который включает:

 * Имя
 * Роль
 * Список предпочтений (каждая запись содержит *название* и *описание*)
 * Список исключений (каждая запись содержит *название* и *описание*)

2. **Главный экран:**

 * Отображать имя пользователя.
 * При клике по имени пользователя — переходить на экран профиля.
 ...
 * Отображать поля:
  ...
  * Имя
  * Роль
  * Списки предпочтений и исключений.
 * Для списков:

  * Кнопка **«Добавить запись»** → открывает модальное окно для добавления записи.
  * Для каждой записи: кнопки **«Редактировать»** и **«Удалить»**.

4. **Интеграция:**

 * Вынести данные о пользователе из `@agent_interface.dart`.
 * Добавить в интерфейс поле **Пользователь с профилем пользователя**.
 * Использовать данные профиля в реализациях интерфейса (там, где они требуются).
...
---
Создай нового агента используя наработки для агентов в проекте (интерфейсы)
Функции агента:
- Анализ переданного проекта (папки). Цель анализа - дать полную картину правильности написания кода в этом проекте.
- Основные критерии анализа - правильность кода, читаемость кода, анализ зависимостей, анализ общей архитектуры. (так же добавь сюда критерии анализа которые используют агенты для кодинга)
- Выходной формат - Json с полями: 1. Общий текст по проведенному анализу 2.Предложение - список файлов- список улучшений (опционально) - список проблем (опционально) 3. Обобщенные предложения по проекту.
---

Вот готовый промпт для твоего запроса, который можно использовать как системное или разработческое задание для нового агента:

---

**Промпт для создания нового агента**

Создай нового агента, используя существующие интерфейсы проекта.

### Задачи агента:

1. Получать на вход проект (папку с файлами кода).

2. Выполнять детальный анализ проекта по следующим критериям:

 * **Правильность кода** (валидность, отсутствие синтаксических ошибок, корректность логики).
 * **Читаемость кода** (наименование переменных и функций, форматирование, комментирование).
 * **Анализ зависимостей** (внешние библиотеки, их количество, версия, актуальность, возможные конфликты).
 * **Анализ архитектуры** (структура проекта, разделение на модули, уровень связности и повторного использования).
 * **Качество тестов** (наличие unit/integration тестов, покрытие).
 * **Безопасность** (потенциальные уязвимости, использование небезопасных библиотек или функций).
 * **Производительность** (наличие потенциально узких мест, избыточные операции, неэффективные алгоритмы).
 * **Соответствие лучшим практикам** (например, SOLID, DRY, KISS, чистая архитектура).

3. Формировать результат в формате **JSON** следующей структуры:
...
```json
{
  ...
  "files": [
    {
      "file": "имя_файла",
      "suggestions": ["список улучшений (опционально)"],
      "problems": ["список проблем (опционально)"]
    }
  ],
  "project_suggestions": [
    "Обобщенные предложения по проекту"
  ]
}
```
...
🔥 День 29. Chain of Agents

Сделайте 4 агента, каждый из которых работает последовательно и использует результат работы прошлого агента

Результат: Агент, делающий комплексную историю (скажем создание -> анализ -> прототипирование -> итоговый результат)
Формат: Видео + Код

---
Описание интерфейса нового редактора

### Основные зоны:

1. **Левая панель**

 * Блок с **деревом файловой системы**.
 * Отображает папки и файлы (иконки папок и файлов).
 * Возможность раскрывать/сворачивать папки.
 * При клике на файл — содержимое открывается в центральной области.

2. **Центральная панель**

 * **Редактор кода/просмотра**.
 * Подсветка синтаксиса.
 * Возможность редактирования.
 * Вкладки сверху для переключения между несколькими открытыми файлами.

3. **Правая панель**

 * **Диалог с агентом**.
 * Верхняя часть — история сообщений (чат в стиле мессенджера).
 * Нижняя часть — поле ввода текста + кнопка **Отправить**.
 * Возможность переключения между агентами (например, вкладки или выпадающий список).

### Пример структуры окна:

```
---------------------------------------------------------
| [Файлы]     |              [Редактор]                | [Агент] 
|             |---------------------------------------|--------- 
|             |   Вкладки: file1.py | file2.js        | Сообщения
|   Дерево    |---------------------------------------| агента
|   файлов    |  >>> содержимое выбранного файла >>>  | ---------
|             |                                       | Поле ввода
|             |                                       | [Отправить]
---------------------------------------------------------
```
---
Вот промпт, который можно использовать для настройки такого оркестратора:

---

**Промпт для оркестратора**

Ты — оркестратор. Твоя задача: принимать от пользователя запросы и управлять их выполнением через подключённых агентов.

Требования к работе:

1. При получении запроса от пользователя:

 * Проанализируй его.
 * Сформируй подробный план выполнения задачи, разбив её на элементарные шаги.

2. Для каждого шага:

 * Определи, какой из агентов способен выполнить действие (используя capability агентов).
 * Сформируй и отправь этому агенту корректный запрос. Если нужно — добавь JSON с данными.
 * Получи результат работы агента.

3. Отображай весь процесс в виде сообщений в чате:

 * Планирование и рассуждения.
 * Отправка данных агенту.
 * Получение результата.
 * Итоговый ответ пользователю.

Его цель — чтобы пользователь видел полный процесс рассуждений и выполнения задачи, а также получил готовый результат.

---

Хочешь, я оформлю это как **системный промпт** (с чёткой инструкцией для LLM), или как **техническое ТЗ** для разработчика, чтобы он реализовал оркестратор?
---