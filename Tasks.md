...
**1. Анализ и настройка зависимостей:**
   - [ ] Добавить необходимые пакеты в `pubspec.yaml`: `http` для API-запросов, `path_provider` для работы с файлами, `audioplayers` для воспроизведения аудио, `permission_handler` для запроса разрешений.
   - [ ] Убедиться, что есть класс или модуль для работы с Yandex Cloud API (уже должен быть для LLM, возможно, потребуется общий клиент с аутентификацией через IAM-токен или API-ключ).

**2. Реализация функции распознавания речи (STT):**
   - [ ] Создать метод `Future<String> _recognizeSpeech(String audioFilePath)`.
   - [ ] Метод должен принимать путь к WAV-файлу.
   - [ ] Сформировать правильный POST-запрос к эндпоинту STT: `https://stt.api.cloud.yandex.net/speech/v1/stt:recognize`.
   - [ ] В заголовках (`headers`) указать Authorization: `Api-Key <YOUR_API_KEY>`.
   - [ ] Тело запроса (`body`) — закодированный binary-файл WAV.
   - [ ] Обработать ответ и извлечь распознанный текст из JSON-поля `result`.
   - [ ] Возвращать этот текст агенту для последующей отправки в LLM.

**3. Реализация функции синтеза речи (TTS):**
   - [ ] Создать метод `Future<String> _synthesizeSpeech(String text)`.
   - [ ] Метод должен принимать текст для озвучивания.
   - [ ] Сформировать POST-запрос к эндпоинту TTS: `https://tts.api.cloud.yandex.net/speech/v1/tts:synthesize`.
   - [ ] В заголовках указать аналогичный API-ключ.
   - [ ] Передать в параметрах запроса (`queryParameters` или `body`) текст, голос (напр., `alena`), формат выходного аудио (напр., `lpcm` или `oggopus`).
   - [ ] Получить из ответа бинарные данные аудиофайла.
   - [ ] Сохранить их в临时ный файл в локальном хранилище устройства и вернуть путь к этому файлу.
   - [ ] Воспроизвести полученный файл с помощью `audioplayers`.
...
**4. Интеграция с агентом и логикой приложения:**
   ...
   - [ ] Все новые функции и UI-элементы должны быть обернуты в условие `if (selectedLlmProvider == "yandex")`.

**5. Обновление пользовательского интерфейса (UI):**
   - [ ] **Экран рассуждающей модели:** Добавить кнопку записи аудио (иконка микрофона) слева от текстового поля ввода.
   - [ ] Реализовать логику записи аудио (можно использовать пакет `sound_recorder` или аналогичный), сохранять запись в WAV-формате и передавать путь к файлу агенту.
   - [ ] **Карточки ответа агента:** Добавить кнопку "воспроизвести" (иконка динамика) на каждую карточку с ответом.
   - [ ] По нажатию на кнопку отправить текст из карточки в метод `_synthesizeSpeech()` и воспроизвести результат.

**6. Тестирование и обработка ошибок:**
   - [ ] Написать проверки на наличие сетевого соединения.
   - [ ] Реализовать обработку ошибок API Yandex (неверный ключ, квоты, формат файла).
   - [ ] Убедиться, что разрешения на запись аудио и доступ к хранилищу запрашиваются и обрабатываются корректно.
