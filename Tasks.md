...
Необходимо дополнить mcp сервер в отдельной папке основного проекта ai-intensive
добавь функциональность сервера:
позволяет получать информацию с публичных телеграм каналов с виде списка.
...
```
Имеет методы для вызова служебных действий и вызова инструментов
### Методы
- `initialize`
