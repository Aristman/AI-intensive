...
class _SettingsScreenState extends State<SettingsScreen> {
  ...
  void dispose() {
    ...
    _yandexTokensController.dispose();
    _deepseekTokensController.dispose();
                        ...
                        children: [
                          ...
                          DropdownButtonFormField<NeuralNetwork>(
                            ...
                            onChanged: (value) {
                              if (value != null) {
                                setState(() {
                                  _currentSettings = _currentSettings.copyWith(selectedNetwork: value);
                                });
                                widget.onSettingsChanged(_currentSettings);
                              }
                            },
                              ...
                              return Column(
                                ...
                                children: [
                                  Row(
                                    mainAxisAlignment: MainAxisAlignment.spaceBetween,
                                    children: [
                                      Text('Температура: ${value.toStringAsFixed(2)}'),
                                      SizedBox(
                                        width: 220,
                                        child: Slider(
                                          key: const Key('llm_temperature_slider'),
                                          value: value,
                                          min: 0.0,
                                          max: maxTemp,
                                          divisions: divisions,
                  ...
                  Card(
                    child: Padding(
                      ...
                      child: Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                          const Text('Глубина истории', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
                          const SizedBox(height: 8),
                          Row(
                            mainAxisAlignment: MainAxisAlignment.spaceBetween,
                            children: [
                  ...
                  Card(
                    child: Padding(
                      ...
                      child: Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                          const Text('Формат ответа', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
                          const SizedBox(height: 8),
                          DropdownButtonFormField<ResponseFormat>(
                            key: const Key('response_format_dropdown'),
                            value: _currentSettings.responseFormat,
                            items: ResponseFormat.values.map((format) {
                              return DropdownMenuItem<ResponseFormat>(
                                value: format,
                                child: Text(format == ResponseFormat.text ? 'Текст' : 'JSON схема'),
                              );
                            }).toList(),
                            onChanged: (value) {
                              if (value != null) {
                                setState(() {
                                  _currentSettings = _currentSettings.copyWith(responseFormat: value);
                                });
                                widget.onSettingsChanged(_currentSettings);
                              }
                            },
                          ...
                          if (_currentSettings.responseFormat == ResponseFormat.json) ...[
                            const SizedBox(height: 16),
                            const Text('JSON схема (опционально)', style: TextStyle(fontSize: 14, color: Colors.grey)),
                            const SizedBox(height: 8),
                            TextField(
                              ...
                              onChanged: (value) {
                                _currentSettings = _currentSettings.copyWith(customJsonSchema: value.isEmpty ? null : value);
                                widget.onSettingsChanged(_currentSettings);
                              },
                            ),
                          ],
                  ...
                  Card(
                    ...
                    child: Padding(
                      ...
                      child: Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                          const Text('MCP сервер', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
                          const SizedBox(height: 8),
                          CheckboxListTile(
                          ...
                          CheckboxListTile(
                            ...
                            onChanged: (bool? value) {
                              setState(() {
                                ...
                                _currentSettings = _currentSettings.copyWith(enabledMCPProviders: updatedProviders);
                              });
                              widget.onSettingsChanged(_currentSettings);
                            },
                            controlAffinity: ListTileControlAffinity.leading,
                                ...
                                children: [
                                  Row(
                                    children: [
                                      ...
                                      const SizedBox(width: 8),
                                      const Text('Токен GitHub MCP', style: TextStyle(fontSize: 14, fontWeight: FontWeight.bold, color: Colors.black87)),
                                    ],
                                  ),
                                  const SizedBox(height: 8),
                                  Text('Токен для GitHub MCP берется из файла .env.', style: TextStyle(fontSize: 12, color: Colors.grey[700])),
                                  const SizedBox(height: 4),
                                  Text('Пожалуйста, добавьте GITHUB_MCP_TOKEN в .env файл для использования GitHub MCP.', style: TextStyle(fontSize: 12, color: Colors.grey[700])),
                                  const SizedBox(height: 8),
                                  Row(
                                    children: [
                                      Icon(_isGithubTokenValid ? Icons.check_circle : Icons.error, color: _isGithubTokenValid ? Colors.green : Colors.red, size: 16),
                                      const SizedBox(width: 4),
                                      Text(
                                        _isGithubTokenValid ? '✅ Токен найден и действителен' : '❌ Токен не найден или недействителен',
                                        style: TextStyle(fontSize: 12, color: _isGithubTokenValid ? Colors.green : Colors.red),
                                      ),
                                    ],
                                  ),
                                ],
