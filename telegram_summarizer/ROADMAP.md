...
- [ ] Создай агента по аналогии с агентом из sample_app (рассуждающая модель)
- [ ] Импорт минимального usecase по образцу `sample_app` через API‑Key, пока без IAM + `x-folder-id`).
