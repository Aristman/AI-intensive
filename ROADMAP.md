...
- [ ] AutoFix (M2)
  ...
  - [x] Оптимизация токенов:
    - [x] Стриппинг комментариев per extension (`.dart/.kt/.java/.md`)
    - [x] Оценка токенов и бюджетирование перед запросом к LLM
    - [x] Селективные выдержки: head + окна вокруг issues (±N строк)
    - [x] Кеширование превью по подписи файла и ключу issues; пропуск неизменившихся файлов
...
- [x] Добавить функциональность подсчета токенов на экране auto_fix при работе агентов: показывать на экране и логгировать расходы токенов.
  - [x] Расширить интерфейс LLM для захвата usage (inputTokens, completionTokens, totalTokens)
  - [x] Обновить агенты для сбора и передачи токенов в meta поля AgentResponse/AgentEvent
  - [x] Модифицировать CodeOpsScreen для отображения токенов и логирования
  - [x] Добавить юнит-тесты и обновить документацию
