...
- [ ] Инструмент выполнения Java-кода в контейнере (`docker_exec_java`)
  - Вход: `filename`, `entrypoint` (main class), `classpath`, доп. флаги
  - Передача кода: volume-монтаж (предпочтительно) или `docker cp`; рабочая директория и политика очистки
...
- [ ] CodeOps: выполнение кода через Docker
  - Кнопка «Запустить в Docker» для JSON‑генерации/вставленного кода; подтверждение перед выполнением
  - Отображение stdout/stderr, кода возврата, таймаута/отмены; аккуратные логи в UI
  - Согласовать и применить JSON‑схему запуска (filename, entrypoint, classpath, флаги)
  - Очистка временных файлов/рабочих директорий после выполнения
- [ ] E2E‑тест потока CodeOps: генерация → подтверждение → запуск в контейнере → сбор логов/результата (smoke‑тест)
