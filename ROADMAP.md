...
- [ ] AutoFix (M2)
  - [x] Интеграция LLM для анализа и генерации патчей (Dart/Markdown/Java/Kotlin)
  ...
  - [x] Парсер unified diff для LLM‑ответов → агрегирование `llm_patches` по файлам (ограничение: простые заголовки, стандартные hunks)
  - [x] Экран: флажок «Include LLM patches» — опционально добавляет `llm_patches` в итоговый список `patches`
  - [x] PatchApplyService: поддержка применения простых full‑file unified diff, если `newContent` отсутствует
  ...
  - [ ] Расширить тесты: негативные кейсы парсинга/применения; виджет‑тесты UI‑переключателя и предпросмотра
